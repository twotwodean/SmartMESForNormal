...
function ProductionForm({ targets }: { targets: WorkOrderRow[] }) {
  ...
  async function submit() {
    const res = await fetch("/api/production/results", {
      ...
      body: JSON.stringify({ workOrderId: selected.id, goodQty: good, defectQty: defect, downtimeMin: downtime }),
    });
    if (res.ok) {
...
export function ProductionClient({ targets }: { targets: WorkOrderRow[] }) {
  return (
    <ToastProvider>
      ...
      {targets.length === 0 ? (
        ...
        <ProductionForm targets={targets} />
      )}
    </ToastProvider>
  );
}
