...
export async function POST(req: Request) {
  ...
  if (
    ...
    typeof body?.qtyPer !== "number"
  ) {
    return NextResponse.json({ error: "parentId·childId·qtyPer가 필요합니다." }, { status: 400 });
  }
  try {
    const r = await addBomComponent({ parentId: body.parentId, childId: body.childId, qtyPer: body.qtyPer });
